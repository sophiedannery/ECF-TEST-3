...
<main>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $passage_emp = $_POST['passage_emp'];

        // Reformater la date et l'heure dans le format MySQL
        $passage_emp_mysql = date('Y-m-d H:i:s', strtotime($passage_emp));


        // Requête de mise à jour
        $sql_update = "UPDATE animaux2 SET nourriture_donnee = ?, grammage_donnee = ?, passage_emp = ? WHERE id = ?";
        try {
            $sth_update = $cx->prepare($sql_update);
            $sth_update->execute([$nourriture_donnee, $grammage_donnee, $passage_emp, $id]);
                    ...
                    <?php foreach ($animaux as $animal) { ?>
                        <tr>
                            <form method="post" action="">
                                ...
                                <td><input type="text" name="nourriture_donnee" value="<?php echo htmlspecialchars($animal['nourriture_donnee']); ?>"></td>
                                <td><input type="text" name="grammage_donnee" value="<?php echo htmlspecialchars($animal['grammage_donnee']); ?>"></td>
                                <td><input type="datetime-local" name="passage_emp" value="<?php echo htmlspecialchars($animal['passage_emp']); ?>"></td>
